     ...
     */    
      ...
      {  
            ...
            if ($limit > 0)
            {
                ...
                $query->select('a.file_title AS title, a.metadesc, a.metakey, a.author, a.date_added AS created, a.language, a.custom_field_6, a.custom_field_7, a.custom_field_8, a.custom_field_9, a.custom_field_10');
                ...
                if (isset($list))
                {
                    foreach($list as $key => $item)
                    {
                        if ($jlistConfig['view.detailsite']){
                            ...
                            // we will use for this version the same link as above
                            // but perhaps should we find here another solution? 
                            $list[$key]->href = JDownloadsHelperRoute::getDownloadRoute($item->slug, $item->catslug, $item->language);
                            // use a link to the category
                            // $list[$key]->href = JDownloadsHelperRoute::getCategoryRoute($item->catslug);
                        }    
                    }
                }
                $rows[] = $list;
