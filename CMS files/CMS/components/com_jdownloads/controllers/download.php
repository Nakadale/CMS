...
class jdownloadsControllerDownload extends JControllerForm
{
    ...
    public function delete()
    {
        ...
        if ($data->params->get('access-delete') == true){
            ...
            if ($id > 0){
                // delete the download
                $result = jdownloadshelper::deleteDownload($id);
            }    
        } else {
            // no permissions - do nothing
            JError::raiseWarning(403, JText::_('JERROR_ALERTNOAUTHOR'));
    ...
    public function send()
    {
        ...
        if (count($files) > 1){
            ...
            if (!$download_in_parts){            
                ...
                if ($jlistConfig['use.alphauserpoints'] && $aup_exist){
                    if ($jlistConfig['use.alphauserpoints.with.price.field']){
                        ...
                        if ($profil->points >= $sum_points){
                            foreach($files as $aup_data){
                                ...
                                if ($price = (int)$db->loadResult()){
                                    $can_download = JDHelper::setAUPPointsDownloads($user->id, $aup_data->file_title, $aup_data->file_id, $price);
                                }
                            }
                        }
                    } else {
                        ...
                        if ($profil->points >= $sum_points){
                            foreach($files as $aup_data){
                                $can_download = JDHelper::setAUPPointsDownloads($user->id, $aup_data->file_title, $aup_data->file_id, 0);
                            }
                        } else {
                            $can_download = false;
                        }    
           ...
           if (!$download_in_parts){
               $can_download = JDHelper::setAUPPointsDownload($user->id, $files[0]->file_title, $files[0]->file_id, $price);
