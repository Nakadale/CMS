...
class JDHelper
{
    ...
    */
    public static function getMenuItemids($catid = 0){
        ...
        if ($catid > 0){
            ...
            $jdItemids['upper'] = $jdItemids['root'];
        }    
    ...
    */
     ...
     *          boolean     $show_empty_categories  the selected value in menu item or configuration
     *
     * @return  array
     *
     */
    public static function getCategoriesList($show_empty_categories, $orderby_pri = '')
    {
        ...
        $query->group('c.id');
        // must be fixed in next beta
        // $query->order($orderCol);
        $query->order('c.lft');
    ...
    */
    public static function buildCategorySelectBox($catlistid, $cat_link_itemids, $root_itemid, $show_empty_categories)
    {
        global $jlistConfig;
    ...
    */
    public static function checkUserPoints($sum_aup_price_points, $marked_files_id)
    {
        ...
        if (file_exists($api_AUP)){
            ...
            if ($profil){
                if ($sum_aup_points > 0 && $sum_aup_points <= $profil->points){
    ...
    * @param mixed $price
    */
    public static function setAUPPointsDownload($user_id, $file_title, $file_id, $price)
    {
        global $jlistConfig;

        if ($jlistConfig['use.alphauserpoints'] && $user_id){
            // We must be sure, that we have not already before reduced the points (possible when user clicked repeatedly on the link)
            $session = JFactory::getSession();
    ...
    * @param mixed $price
    */
    public static function setAUPPointsDownloads($user_id, $file_title, $file_id, $price)
    {
        global $jlistConfig;
