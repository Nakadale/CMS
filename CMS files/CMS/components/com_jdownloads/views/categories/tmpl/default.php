    ...
    if ($layout_cat_text != ''){
        ...
        if(!empty($cats)){
            ...
            for ($i=$this->pagination->limitstart; $i<$amount; $i++) {
               ...
               $sum_subcats = count($cats[$i]->getChildren()); 
               ...
               if ($cats[$i]->pic != '' ) {
                   $catpic = '<a href="'.$catlink.'"><img src="'.JURI::base().'images/jdownloads/catimages/'.$cats[$i]->pic.'" align="top" width="'.$jlistConfig['cat.pic.size'].'" height="'.$jlistConfig['cat.pic.size.height'].'" border="0" alt="" /></a> ';
               } else {
                   $catpic = '';
               }
                 ...
                 if ($layout->cols > 1 && strpos($layout_cat_text, '{cat_title1}')){
                    ...
                    for ($a=0; $a < $layout->cols; $a++){
                         ...
                         if ( $i < $amount ){
                            ...
                            if (!$sum_subcats){
                                   ...
                                   $html_cat = str_replace("{sum_subcats$x}", JText::_('COM_JDOWNLOADS_FRONTEND_COUNT_SUBCATS').' '.$sum_subcats, $html_cat);
                            }
                               
                            if (!$cats[$i]->numitems){
                                   $html_cat = str_replace("{sum_files_cat$x}", '', $html_cat);
                            } else {
                                   $html_cat = str_replace("{sum_files_cat$x}", JText::_('COM_JDOWNLOADS_FRONTEND_COUNT_FILES').' '.$cats[$i]->numitems, $html_cat);
                            }                               
                    ...
                    for ($b=1; $b < 10; $b++){
                     ...
                     if ($sum_subcats == 0){
                        ...
                        $html_cat = str_replace('{sum_subcats}', JText::_('COM_JDOWNLOADS_FRONTEND_COUNT_SUBCATS').' '.$sum_subcats, $html_cat);
                     }
                     $html_cat = str_replace('{sum_files_cat}', JText::_('COM_JDOWNLOADS_FRONTEND_COUNT_FILES').' '.$cats[$i]->numitems, $html_cat);
                 }
