 ...
 */
 ...
 defined('_JEXEC') or die('Restricted access');
    ...
    $total_downloads            = $this->category->numitems;
    ...
    if ($header != ''){
        ...
        // build home link        
        $home_link = '<a href="'.JRoute::_('index.php?option=com_jdownloads&amp;Itemid='.$menuItemids['root']).'">'.'<img src="'.JURI::base().'components/com_jdownloads/assets/images/home_fe.png" width="32" height="32" border="0" alt="" /></a> <a href="'.JRoute::_('index.php?option=com_jdownloads&amp;Itemid='.$menuItemids['root']).'">'.JText::_('COM_JDOWNLOADS_HOME_LINKTEXT').'</a>';
        // build search link
        $search_link = '<a href="'.JRoute::_('index.php?option=com_jdownloads&amp;view=search&amp;Itemid='.$menuItemids['search']).'">'.'<img src="'.JURI::base().'components/com_jdownloads/assets/images/search.png" width="32" height="32" border="0" alt="" /></a> <a href="'.JRoute::_('index.php?option=com_jdownloads&amp;view=search&amp;Itemid='.$menuItemids['search'].'').'">'.JText::_('COM_JDOWNLOADS_SEARCH_LINKTEXT').'</a>';
        // build frontend upload link
        $upload_link = '<a href="'.JRoute::_('index.php?option=com_jdownloads&amp;view=form&amp;layout=edit&amp;Itemid='.$menuItemids['upload']).'">'.'<img src="'.JURI::base().'components/com_jdownloads/assets/images/upload.png" width="32" height="32" border="0" alt="" /></a> <a href="'.JRoute::_('index.php?option=com_jdownloads&amp;view=form&amp;layout=edit&amp;Itemid='.$menuItemids['upload'].'').'">'.JText::_('COM_JDOWNLOADS_UPLOAD_LINKTEXT').'</a>';
        ...
        if ($menuItemids['upper'] > 1){   // 1 is 'root'
            ...
            $header = str_replace('{upper_link}', '<a href="'.$upper_link.'">'.'<img src="'.JURI::base().'components/com_jdownloads/assets/images/upper.png" width="32" height="32" border="0" alt="" /></a> <a href="'.$upper_link.'">'.JText::_('COM_JDOWNLOADS_UPPER_LINKTEXT').'</a>', $header);    
        } else {
    ...
    if ($cat_layout_text != ''){
        ...
        if ($this->category->pic != '' ) {
            $catpic = '<img src="'.JURI::base().'images/jdownloads/catimages/'.$this->category->pic.'" align="top" width="'.$jlistConfig['cat.pic.size'].'" height="'.$jlistConfig['cat.pic.size.height'].'" border="0" alt="" /> ';
        } else {
            $catpic = '';
        }
        ...
        if($total_subcategories > 0){
            ...
            for ($i=0; $i < $total_subcategories; $i++){
                ...
                $xnumitems = $this->children[$parentCatid][$i]->getNumItems(true);
                $countchildren = count($this->children[$parentCatid][$i]->getChildren());
                ...
                if ($show_empty_categories || $jlistConfig['view.empty.categories'] || $this->children[$parentCatid][$i]->getNumItems(true) || count($this->children[$parentCatid][$i]->getChildren())){            
                    ...
                    //display cat info
                     ...
                     if ($this->children[$parentCatid][$i]->pic != '' ) {
                         $catpic = '<a href="'.$catlink.'"><img src="'.JURI::base().'images/jdownloads/catimages/'.$this->children[$parentCatid][$i]->pic.'" align="top" width="'.$jlistConfig['cat.pic.size'].'" height="'.$jlistConfig['cat.pic.size.height'].'" border="0" alt="" /></a> ';
                     } else {
                         $catpic = '';
                     }                         
                     ...
                     if ($cats_layout->cols > 1 && strpos($cats_layout_text, '{cat_title1}')){
                        ...
                        for ($a=0; $a < $cats_layout->cols; $a++){
                            ...
                            if ($this->children[$parentCatid][$i]->pic != '' ) {
                             ...
                             if ($i < count($this->children[$this->category->id])){
                                ...
                                if ($countchildren == 0){
                                    $body_subcats = str_replace("{sum_subcats$x}", JText::_('COM_JDOWNLOADS_FRONTEND_COUNT_SUBCATS').' 0', $body_subcats); 
                                } else {
                                    $body_subcats = str_replace("{sum_subcats$x}", JText::_('COM_JDOWNLOADS_FRONTEND_COUNT_SUBCATS').' '.$countchildren, $body_subcats);
                                }
                        ...
                        for ($b=1; $b < 10; $b++){
                         ...
                         if ($countchildren == 0){
                            ...
                            $body_subcats = str_replace('{sum_subcats}', JText::_('COM_JDOWNLOADS_FRONTEND_COUNT_SUBCATS').' '.$countchildren, $body_subcats);
                         }
    ...
    if ($layout_files_text != ''){
        ...
        for ($i = 0; $i < count($files); $i++) {
            ...
            if ($jlistConfig['view.detailsite']){
                ...
                // Symbol anzeigen - auch als url
                if ($files[$i]->file_pic != '' ) {
                    $filepic = '<a href="'.$title_link.'">'.'<img src="'.JURI::base().'images/jdownloads/fileimages/'.$files[$i]->file_pic.'" align="top" width="'.$jlistConfig['file.pic.size'].'" height="'.$jlistConfig['file.pic.size.height'].'" border="0" alt="" /></a> ';
                } else {
                    $filepic = '';
                }
                ...
                if ($user_can_seen_download_url){
                   ...
                   if ($files[$i]->file_pic != '' ) {
                        $filepic = '<a href="'.$download_link.'"><img src="'.JURI::base().'images/jdownloads/fileimages/'.$files[$i]->file_pic.'" align="top" width="'.$jlistConfig['file.pic.size'].'" height="'.$jlistConfig['file.pic.size.height'].'" border="0" alt="" /></a>';
                   } else {
                        $filepic = '';
                   }
                   $html_file = str_replace('{file_pic}',$filepic, $html_file);
                   ...
                   $html_file = str_replace('{file_title}', $download_link_text.' '.$editIcon, $html_file);
                    ...
                    if ($files[$i]->file_pic != '' ) {
                        $filepic = '<img src="'.JURI::base().'images/jdownloads/fileimages/'.$files[$i]->file_pic.'" align="top" width="'.$jlistConfig['file.pic.size'].'" height="'.$jlistConfig['file.pic.size.height'].'" border="0" alt="" />';
                    } else {
                        $filepic = '';
                    }
                    $html_file = str_replace('{file_pic}',$filepic, $html_file);
        ...
        if (isset($user_can_seen_download_url)){ 
            ...
            if ($total_downloads > 0){
                $body_cat = str_replace('{checkbox_top}', '<div style="text-align:center; padding:8px;"><img src="'.JURI::base().'components/com_jdownloads/assets/images/info32.png" align="middle" width="32" height="32" border="0" alt="" /> '.$regg.'</div>', $body_cat);                    
            } else {
                $body_cat = str_replace('{checkbox_top}', '', $body_cat);                    
            }    
        }
