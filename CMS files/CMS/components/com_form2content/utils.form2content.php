...
class F2C_AdminHelper
{
	static function generateSampleTemplate($contentTypeId)
	{
		...
		$db->setQuery("SELECT title FROM #__f2c_project WHERE id=$contentTypeId");
		...
		$db->setQuery("SELECT fieldname, fieldtypeid FROM #__f2c_projectfields WHERE projectid=$contentTypeId ORDER BY ordering");
		...
		for ($i=0, $n=count($rows); $i < $n; $i++) 
	    {
	      	...
	      	switch(intval($row->fieldtypeid))
	      	{
	      		case F2C_FIELDTYPE_SINGLESELECTLIST: 
	      			$template .= $row->fieldname.' (value): {$'.strtoupper($row->fieldname)."}<br/>\n";			
	      			$template .= $row->fieldname.' (text): {$'.strtoupper($row->fieldname)."_TEXT}\n";
	      			break;
	      			
	      		default:
	      			$template .= $row->fieldname.': {$'.strtoupper($row->fieldname)."}\n";			
	      			break;
	      	}
